...
$target = if ($env:DOTNET_FORMAT_TARGET) {
  $env:DOTNET_FORMAT_TARGET
} else {
  $slnx = Get-ChildItem -Path $repoRoot -Filter *.slnx -File | Select-Object -First 1
  if ($slnx) {
    $slnx.FullName
  } else {
    $sln = Get-ChildItem -Path $repoRoot -Filter *.sln -File | Select-Object -First 1
    if (-not $sln) {
      throw "No .slnx or .sln file found at repo root. Set DOTNET_FORMAT_TARGET explicitly."
    }
    $sln.FullName
  }
}
