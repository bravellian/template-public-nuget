...
```bash
bash .codex/skills/dotnet-test-triage/scripts/run-test-triage.sh "$(ls *.slnx | head -n1)" --no-restore
```

```powershell
pwsh -File .codex/skills/dotnet-test-triage/scripts/run-test-triage.ps1 (Get-ChildItem -Filter *.slnx | Select-Object -First 1).Name --no-restore
```

Optional: override the default command (useful for repo-specific defaults):
```bash
DOTNET_TEST_CMD="dotnet test $(ls *.slnx | head -n1) --no-restore" \
  bash .codex/skills/dotnet-test-triage/scripts/run-test-triage.sh
```

```powershell
$env:DOTNET_TEST_CMD = "dotnet test $((Get-ChildItem -Filter *.slnx | Select-Object -First 1).Name) --no-restore"
pwsh -File .codex/skills/dotnet-test-triage/scripts/run-test-triage.ps1
```
